...
class AutoClickerWithJigglerGUI:
    def __init__(self, master):
        ...
        # Add trace to handle language changes
        def on_language_change(*args):
            current_language = self.language.get()
            # If display name is used, convert to code
...
We appreciate your feedback""",
    ...
    def update_all_widgets_language(self, parent, current_language):
        ...
        # Track number of widgets processed
        widgets_processed = 0
        
        # Comprehensive mapping of text variations and their translation keys
        text_mappings = {
            # Labels and Buttons
            "labels_buttons": [
                {
            ...
            # Text widgets and long descriptions
            "text_descriptions": [
                {
                    "cs_texts": [
    ...
    def update_language(self, *args):
        ...
        try:
            ...
            # Recursively update widgets in each tab
            ...
            # Update specific language-dependent variables
            language_vars = {
            ...
            for var_name, translation_key in language_vars.items():
                try:
                    # Use getattr to safely retrieve the variable
                    var = getattr(self, var_name, None)
                    
                    # Check if var is a StringVar or similar
                    if hasattr(var, 'set'):
                        var.set(self.translations[current_language][translation_key])
                    elif isinstance(var, str):
                        # If it's a string, we might need to update a different way
                        setattr(self, var_name, 
                                self.translations[current_language][translation_key])
                except Exception as var_error:
                    log_message(f"Error updating variable {var_name}: {var_error}")
            
            # Attempt to save settings with fallback
            try:
                # Check if save_settings method exists and is callable
            ...
            # Force a redraw of the entire application
